...
- (void)viewDidLoad {
    ...
    _titles = @[@"ScanDevices", @"MyDevices", @"ConfigDevices", @"MyFamilies", @"IRCodeTest"];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"MyDeviceListView"]) {
        ...
        if ([target isKindOfClass:[MyDeviceListViewController class]]) {
            ...
            myDeviceVC.myDevices = [[DeviceDB sharedOperateDB] readAllDevicesFromSql];
        }
    }
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    switch (indexPath.row) {
        ...
        case 2: {
            ...
            break;
        case 3: {
            [self performSegueWithIdentifier:@"FamilyListView" sender:nil];
        }
            break;
        case 4: {
            [self performSegueWithIdentifier:@"IRCodeTestView" sender:nil];
        }
            break;
        default:
            break;
